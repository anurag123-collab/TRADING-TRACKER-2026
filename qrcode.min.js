/**
 * QRCode.js - Fast Standalone Offline QR Code Generator
 * License: MIT
 */
(function(window){
    var QRCode = function(el, opt) {
        ...
        this._htOption = {
            width: opt.width || 256,
            height: opt.height || 256,
            typeNumber: 4,
            colorDark: opt.colorDark || "#000000",
            colorLight: opt.colorLight || "#ffffff",
            correctLevel: opt.correctLevel || 1
        };
        if (typeof el === "string") el = document.getElementById(el);
        this._el = el;
        this._oQRCode = null;
        if (opt.text) this.makeCode(opt.text);
    };

    // Helper QRCode generator logic
    ...
    var QRUtil = {
        ...
        getLostPoint: function(qrModel) {
            var moduleCount = qrModel.getModuleCount();
            var lostPoint = 0;
            for (var row = 0; row < moduleCount; row++) {
                for (var col = 0; col < moduleCount; col++) {
                    var sameCount = 0;
                    var dark = qrModel.isDark(row, col);
                    for (var r = -1; r <= 1; r++) {
                        if (row + r < 0 || moduleCount <= row + r) continue;
                        for (var c = -1; c <= 1; c++) {
                            if (col + c < 0 || moduleCount <= col + c) continue;
                            if (r == 0 && c == 0) continue;
                            if (dark == qrModel.isDark(row + r, col + c)) sameCount++;
                        }
                    }
    ...
    for (var i = 8; i < 256; i++) QRMath.EXP_TABLE[i] = QRMath.EXP_TABLE[i - 4] ^ QRMath.EXP_TABLE[i - 5] ^ QRMath.EXP_TABLE[i - 8] ^ QRMath.EXP_TABLE[i - 2];
    ...
    QRCode.prototype.makeCode = function(sText) {
        var typeNum = 4;
        if (sText.length > 50) typeNum = 8;
        if (sText.length > 120) typeNum = 12;
        this._oQRCode = new QRCodeModel(typeNum, this._htOption.correctLevel);
